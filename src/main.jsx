...
function App(){
  const [authed,setAuthed]=useState(null); const [user,setUser]=useState(null); const [subjects,setSubjects]=useState([]); const [groups,setGroups]=useState([]); const [active,setActive]=useState(null); const [elapsed,setElapsed]=useState(0); const [loading,setLoading]=useState(false); const [tab,setTab]=useState('home'); const [err,setErr]=useState('');
  async function hydrate(seed){ setLoading(true); setErr(''); try{ const info=seed?.s?seed:await ypt.reloadInfo(); setUser(info); setSubjects(pickSubjects(info).filter(s=>!s.archived)); try{setGroups(flattenGroups(await ypt.myGroups()))}catch{} setAuthed(true); }catch(e){ setErr(e.message); if(/401|Not signed in/i.test(String(e.message))) setAuthed(false); }finally{setLoading(false)} }
  ...
  {tab==='stats'&&<section><h3>Statistics</h3><div className="statGrid"><div className="stat"><span>Subjects</span><b>{subjects.length}</b></div><div className="stat"><span>Groups</span><b>{groups.length}</b></div><div className="stat"><span>Date</span><b>{today()}</b></div></div><p className="muted block">This build keeps YPT authentication server-side in an HttpOnly cookie and proxies the supported YPT calls through the Node server.</p></section>}
  </main></div>
}
