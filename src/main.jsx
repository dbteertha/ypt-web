...
function App(){
  ...
  {tab==='stats'&&<section><h3>Statistics</h3><div className="statGrid"><div className="stat"><span>Today</span><b>{fmt(todayMs)}</b></div><div className="stat"><span>Subjects</span><b>{subjects.length}</b></div><div className="stat"><span>Groups</span><b>{groups.length}</b></div><div className="stat"><span>Date</span><b>{today()}</b></div></div><p className="muted block">Today's total is loaded from your YPT day log. Use Sync after studying from another device to refresh it.</p></section>}
  </main></div>
}
